...
- **Public client config**: `GET /api/config/` (`views.config`) returns a *whitelisted* dict of client-facing settings — currently `{cash_enabled, custom_amount_enabled, negative_balance_limit, shelly_configured}`. Never widen it to dump the whole `Setting` table; it holds the Shelly credentials. The SPA fetches it on login and caches it in `localStorage` (`PiikkiOffline` key `config`) so config-driven UI still renders offline. `shelly_configured` is used to show Shelly error toasts when sessions are started/ended offline.
