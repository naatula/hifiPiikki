...
- **Public client config**: `GET /api/config/` (`views.config`) returns a *whitelisted* dict of client-facing settings — currently `{cash_enabled, custom_amount_enabled, negative_balance_limit, organization_name}`. Never widen it to dump the whole `Setting` table; it holds the Shelly credentials. The SPA fetches it on login and caches it in `localStorage` (`PiikkiOffline` key `config`) so config-driven UI still renders offline.
- **Negative balance limit**: The `negative_balance_limit` Setting (positive decimal, e.g. `50` = balance can't go below −50 €) blocks purchases server-side when the resulting balance would exceed the cap. `Tab.ignore_balance_limit` (boolean, default False) exempts individual tabs. The client also pre-checks locally (using cached balance from `tabsById`) before queueing offline purchases; if a replay is later rejected server-side, it appears as a failed item in the offline queue section of the statistics panel.
- **Organization name**: The `organization_name` Setting customises the login title and footer in the SPA. When empty or unset, defaults to "hifiPiikki".
